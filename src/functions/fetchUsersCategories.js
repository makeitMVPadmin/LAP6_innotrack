...
const fetchUsersCategories = async ({ limit = null, random = false, max = null } = {}) => {
    try {
        ...
        const categoriesPromises = users.map(async (user) => {
            ...
            const query = db.collection("categories").where("userId", "==", user.id);
...
async function test() {
    try {
        ...
        const categories = await fetchUsersCategories({ limit: 2, random: true, max: 10 });
